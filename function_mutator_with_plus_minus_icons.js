    ...
    registerMutators: function () {
      ...
      Blockly.Blocks['custom_function'] = {
        ...
        updateShape_: function (count = this.argumentCount_) {
          ...
          for (let i = 0; i < this.argumentCount_; i++) {
            ...
            if (input && input.connection && input.connection.targetBlock()) {
              ...
              currentArgs.push(connectedBlock);
            } else {
              currentArgs.push(null);
            }
          }
          ...
          for (let i = 0; i < count; i++) {
            const fieldName = this.argumentNames_[i] || 'arg' + i;
            const input = this.appendValueInput('ARG' + i).setCheck('String');
            if (i > 0) {
              input.appendField(',');
            }
            if (currentArgs[i]) {
              input.connection.connect(currentArgs[i].outputConnection);
            }
          }

          this.argumentCount_ = count;
          this.argumentNames_.length = count;

          if (!this.getInput('CLOSE_STATEMENT')) {
            this.appendDummyInput('CLOSE_STATEMENT').appendField('):');
          } else {
            this.moveInputBefore('CLOSE_STATEMENT', null);
          }
        ...
        decompose: function (workspace) {
          ...
          for (let i = 0; i < this.argumentCount_; i++) {
            const argumentBlock = workspace.newBlock('mutator_argument');
        ...
        compose: function (containerBlock) {
          ...
          while (itemBlock) {
            argumentCount.push(itemBlock);
            itemBlock =
              itemBlock.nextConnection &&
              itemBlock.nextConnection.targetBlock();
          }

          this.updateShape_(argumentCount.length);
        },
      ...
      if (!Blockly.Extensions.isRegistered('custom_function')) {
        Blockly.Extensions.registerMutator(
          'custom_function',
          {
            mutationToDom: function () {
              const container =
                Blockly.utils.xml.createElement('functionMutation');
              container.setAttribute('arguments', this.argumentCount_);
              return container;
            },
